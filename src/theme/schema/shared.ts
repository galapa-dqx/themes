...
export const closed = <const P extends Record<string, TSchema>>(
  properties: P,
...
export const constrainedRecord = (pattern: string, value: TSchema) =>
...
export const metadataProperties = (previewImage: TSchema) => ({
  ...
  previewImage: Type.Optional(previewImage),
...
export function createControlSchemas(
  compiled: boolean,
): Readonly<Record<RootControlId, TSchema>> {
  return Object.freeze(
    Object.fromEntries(
      ROOT_CONTROL_IDS.map((root) => [root, controlDocumentSchema(root, compiled)]),
